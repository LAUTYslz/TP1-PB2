...
public class Universidad {
	...
	public Boolean agregarCicloLectivo(CicloLectivo nuevoCiclo) {
		for(CicloLectivo i: ciclosLectivos) {
			if(	i.getFechaInicio().isBefore(nuevoCiclo.getFechaInicio()) && i.getFechaFinalizacion().isAfter(nuevoCiclo.getFechaInicio()) || 
				i.getFechaInicio().isBefore(nuevoCiclo.getFechaFinalizacion()) && i.getFechaFinalizacion().isAfter(nuevoCiclo.getFechaFinalizacion())	){
				return false;
			}
		}
	...
	public boolean agregarCorrelativaAMateria(Integer idMateria, Integer idCorrelativa) {
		...
		if(materiaBuscada==null || correlativaBuscada==null || idMateria==idCorrelativa || materiaBuscada.getCorrelativas().contains(correlativaBuscada)) {
			return false;
		}
...
//	}
	public Boolean agregarCurso(Curso curso) {
        ...
        ListaDias dia = curso.getHorario().getDias();
        ListaTurnos turno = curso.getHorario().getTurno();
        ...
        if (materia != null && cicloLectivo != null && aula != null && existe != true) {
            cursos.add(curso);
            sePudoAgregar = true;
        }
...
private Curso buscarCursoPorId(Integer id) {
	for(Curso i: cursos)
